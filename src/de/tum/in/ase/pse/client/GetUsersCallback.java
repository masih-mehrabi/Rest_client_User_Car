...
public class GetUsersCallback implements Callback<JsonNode> {
    ...
    public void completed(HttpResponse<JsonNode> response) {
       for (int i = 0; i < getUsers(response).length(); i++) {
           if (checkRentalsByUser(getUsers(response), i)) {
               Unirest.get(getRentalLink(getUsers(response), i)).asJsonAsync(new GetRentalCallback(delegate));
               System.out.println(getRentalLink(getUsers(response), i));
           }
       }
    }
