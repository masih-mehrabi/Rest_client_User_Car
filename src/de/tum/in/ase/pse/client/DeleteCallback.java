...
public class DeleteCallback implements Callback<JsonNode> {
    ...
    public void completed(HttpResponse<JsonNode> response) {
        Car carToDelete = new Car(getResponseBody(response).getObject());
        carToDelete.setId(getCarId(response.getBody()));
        delegate.deleteCarFinished(carToDelete, getResponseStatus(response));
    }
